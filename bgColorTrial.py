# routelink_slot_autogen_endp_readonly_linktab.py
"""
RouteLink:
...
- Link Details: ID column removed from display; when opened for a specific route,
  the Drop field is pre-filled with the route's end_point and set readonly (user cannot edit).
  When Link Details is opened generally, Drop is editable.
- Deleting links uses hidden iids to determine IDs.
"""
...
DB = "routelink.db"
LOGO_PATH = "C:/VIT/First Semester/InnoJam/.png"
...
def generate_sample_holidays(year: int, seed: int = 123):
    ...
    fixed = [(1, 26), (5, 1), (8, 15), (10, 2), (12, 25)]
    holidays = set()
    for m, d in fixed:
        try:
...
class RouteLinkApp(tk.Tk):
    def __init__(self):
        ...
        # header
        top = tk.Frame(self,width =400, height=300, bg="lightblue")
        top.pack(fill="x",pady=(10, 0))

        img = Image.open("C:/Users/kaush/Downloads/In Plane Aesthetic.png")
        ...
        if self.logo_photo:
            tk.Label(top, image=self.logo_photo).pack(side="left", padx=(14, 12))
        else:
            tk.Label(top, text="ROUTELINK", font=("Helvetica", 26, "bold"),bg="lightblue").pack(
                side="left", padx=(18, 12))
        brand = tk.Frame(top, bg="lightblue"); brand.pack(side="left", anchor="n")
        ...
        style.configure("TNotebook.Tab", font=("Helvetica", 11, "bold"),bg="lightblue")
...
class CalendarTab(ttk.Frame):
    def __init__(self, parent, app, holidays=None, get_counts=None, get_summary=None):
        ...
        tk.Label(self, text="Hover a day to see route summaries. Click a date to open Route Details.", font=("Arial", 10), fg="#444").pack(pady=(0, 8))
...
class RouteTab(ttk.Frame):
    def __init__(self, parent, app):
        ...
        tk.Label(self, text="Double-click a row to open Link Details for that route (join people there).", font=("Arial", 10), fg="#444").pack(pady=(0, 8))
...
class LinkTab(ttk.Frame):
    def __init__(self, parent, app):
        ...
        tk.Label(self, text="👥 Link Details (Join Route)", font=("Georgia", 20, "bold"), fg="#0b5878").pack(pady=(6, 4))
        tk.Label(self, text="Enter your details and click 'Join Route' to register yourself for the selected route/date.", font=("Arial", 10), fg="#444").pack(pady=(0, 8))
        ...
        # No ID column shown. Visible columns only:
        cols = ("Name", "Drop", "Phone", "Year", "Branch")
        self.tree = ttk.Treeview(self, columns=cols, show="headings", height=10, selectmode="extended")
        for c in cols:
            self.tree.heading(c, text=c)
    ...
    def refresh(self):
        ...
        for row in c.fetchall():
            lid, name, drop, phone, year, branch = row
            iid = f"link_{lid}"
            self.tree.insert("", "end", iid=iid, values=(name, drop, phone, year, branch))
        conn.close()
        self.delete_link_btn.config(state="disabled")
    ...
    def refresh_for_route(self):
        ...
        c = conn.cursor()
        c.execute("""
            SELECT l.id, l.name, l.drop_point, l.phone, l.course_year, l.branch
            FROM links l
    ...
    def join_route(self):
        if not self.route_id or not self.route_date:
            ...
            return
        # If Drop entry is readonly it still provides the correct value via get()
        vals = [self.entries[k].get().strip() for k in self.entries]
        if not all(vals):
        ...
        for e in self.entries.values():
            try:
                ...
                pass
        # If currently in route-specific mode, re-set Drop back to readonly value
        if self.route_end_point:
            drop_entry = self.entries.get("Drop")
            if drop_entry:
                drop_entry.delete(0, tk.END)
