...
class MiniCalendar(tk.Frame):
        ...
        for week in cal.monthdayscalendar(self.year, self.month):
            ...
            for day in week:
                if day == 0:
                    ...
                    if is_today:
                        bg, fg, text = "#4a90e2", "white", str(day)
                    elif is_hol:
                        bg, fg, text = "#ffefef", "#b33", f"{day}\n★"
                    else:
                        bg, fg, text = "#ffffff", "#000", str(day)
                    b = tk.Button(row, text=text, width=6, height=2, bg=bg, fg=fg,
                                  command=lambda d=day: self._on_click(d))
                    b.pack(side="left", padx=1, pady=1)
    ...
    def _on_click(self, day):
        ...
        if date(self.year, self.month, day) < today:
            messagebox.showinfo("Day has passed", "You selected a day that has already passed.")
            return
        if callable(self.on_select):
            self.on_select(iso)
...
class CalendarTab(ttk.Frame):
    ...
    def display_routes_for_date(self, iso_date):
        ...
        c = conn.cursor()
        c.execute("""
            SELECT r.slot_no, r.end_point 
            FROM routes r 
            JOIN calendar c ON r.id=c.route_id 
            WHERE c.travel_date=?""", (iso_date,))
        rows = c.fetchall()
        conn.close()
        if not rows:
            self.routes_text.insert(tk.END, "No routes for this date.\n")
            return
        for r in rows:
            self.routes_text.insert(tk.END, f"Slot: {r[0]}, End Point: {r[1]}\n")
